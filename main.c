...
#define YVO_DB_QUERY_EXISTING_USER "select username from " YVO_DB_USER_TABLE " where username=\'%s\'"
#define YVO_DB_QUERY_EXISTING_USER_PASS "select password from " YVO_DB_USER_TABLE " where username=\'%s\'"
#define YVO_DB_INSERT_USER "insert into " YVO_DB_USER_TABLE " (username, password) VALUES (\'%s\', \'%s\')"
...
void yvo_db_disconnect(PGconn* conn) {
    PQfinish(conn);
}

int yvo_db_register_user(PGconn* conn, const char* username, unsigned int username_len, 
    ...
    snprintf(query_existing_user, sizeof(query_existing_user), YVO_DB_QUERY_EXISTING_USER, username_clean);
    ...
    char sql_insert_user[sizeof(YVO_DB_INSERT_USER) + username_len + password_len];


    snprintf(sql_insert_user, sizeof(sql_insert_user), YVO_DB_INSERT_USER, username_clean, password_clean);
...
int yvo_db_login_user(PGconn* conn, const char* username, unsigned int username_len, 
        const char* password, unsigned int password_len) {
    // if(username_len > YVO_MAX_USERNAME_LENGTH) {
    ...
    char query_existing_user_pass[sizeof(YVO_DB_QUERY_EXISTING_USER_PASS) + username_len];
    snprintf(query_existing_user_pass, sizeof(query_existing_user_pass), YVO_DB_QUERY_EXISTING_USER_PASS, username_clean);
...
int yvo_process_command(struct YVOClientThreadParams* params, char* cmd, unsigned int len) {
    ...
    if(strncmp(cmd, YVO_REGISTER_COMMAND, strlen(YVO_REGISTER_COMMAND)) == 0) {
        ...
        if(yvo_db_register_user(params->conn, username_ptr, username_len, password_ptr, password_len) == -1) {
        ...
        //     return -1;
        // }
        if(yvo_db_login_user(params->conn, username_ptr, username_len, password_ptr, password_len) == -1) {
